...
# File: scripts/compose-down.sh
# Purpose: Launcher entrypoint that discovers and loads every compose/*.yml
#          module (in dependency order) and stops/removes the stack idempotently.
#
# Usage:
#   scripts/compose-down.sh                 # down --remove-orphans
#   scripts/compose-down.sh -f extra.yml    # merge an additional compose file
#   scripts/compose-down.sh -- down -v      # pass arguments to `docker compose`
#
# Module load order:
...
usage() {
  cat <<'USAGE'
Usage: compose-down.sh [-f FILE]... [-- COMPOSE_ARGS...]

  -f, --file FILE   Merge an additional Compose file after the discovered modules.
  --                Pass all remaining arguments to `docker compose`.
  (no args)         Stop and remove the stack (down --remove-orphans).

Examples:
  compose-down.sh
  compose-down.sh -- down --volumes
USAGE
}
...
main() {
  parse_args "$@"
  log_info "Stopping HES compose stack."
  ...
  if [[ ${#PASSTHROUGH_ARGS[@]} -gt 0 ]]; then
    ...
    log_success "HES compose stack is down."
  fi
}
