...
# File: scripts/compose-up.sh
# Purpose: Launcher entrypoint that discovers and loads every compose/*.yml
#          module (in dependency order) and brings the stack up idempotently,
#          replacing the Compose `include` directive for broader compatibility.
#
# Usage:
#   scripts/compose-up.sh                 # validate, pull, and start the stack
#   scripts/compose-up.sh -f extra.yml    # merge an additional compose file
#   scripts/compose-up.sh -- ps           # run an arbitrary compose subcommand
...
usage() {
  cat <<'USAGE'
Usage: compose-up.sh [-f FILE]... [-- COMPOSE_ARGS...]

  -f, --file FILE   Merge an additional Compose file after the discovered modules.
  --                Pass all remaining arguments to `docker compose`.
  (no args)         Validate, pull, and start the stack (up -d --remove-orphans).

Examples:
  compose-up.sh
  compose-up.sh -f compose/observability.yml
  compose-up.sh -- ps
  compose-up.sh -- logs --tail=200 -f
USAGE
}
...
main() {
  parse_args "$@"
  log_info "Starting HES compose stack."
  ...
  if [[ ${#PASSTHROUGH_ARGS[@]} -gt 0 ]]; then
    ...
    log_success "HES compose stack is up."
  fi
}
