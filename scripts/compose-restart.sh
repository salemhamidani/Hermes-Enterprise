...
# File: scripts/compose-restart.sh
# Purpose: Launcher entrypoint that discovers and loads every compose/*.yml
#          module (in dependency order) and restarts the running stack.
#
# Usage:
#   scripts/compose-restart.sh                 # restart all services
#   scripts/compose-restart.sh -f extra.yml    # merge an additional compose file
#   scripts/compose-restart.sh -- restart svc  # pass arguments to `docker compose`
#
# Module load order:
...
usage() {
  cat <<'USAGE'
Usage: compose-restart.sh [-f FILE]... [-- COMPOSE_ARGS...]

  -f, --file FILE   Merge an additional Compose file after the discovered modules.
  --                Pass all remaining arguments to `docker compose`.
  (no args)         Restart the stack (restart).

Examples:
  compose-restart.sh
  compose-restart.sh -f compose/observability.yml
  compose-restart.sh -- restart traefik
USAGE
}
...
main() {
  parse_args "$@"
  log_info "Restarting HES compose stack."
  ...
  if [[ ${#PASSTHROUGH_ARGS[@]} -gt 0 ]]; then
    ...
    run_compose restart
    log_success "HES compose stack restarted."
  fi
}
